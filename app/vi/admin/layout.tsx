...
const navItems = [
  { href: "/vi/admin", icon: LayoutDashboard, label: "Tổng quan" },
  { href: "/vi/admin/users", icon: Users, label: "Quản lý người dùng" },
  { href: "/vi/admin/files", icon: FileText, label: "Quản lý file" },
  { href: "/vi/admin/folders", icon: FolderOpen, label: "Quản lý thư mục" },
  { href: "/vi/admin/analytics", icon: BarChart3, label: "Thống kê & Báo cáo" },
];
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data.role !== "admin") { router.push("/vi/dashboard"); return; }
          setUser(data);
        } else {
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-slate-900/80 border-r border-slate-800 transform transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"} flex flex-col`}>
        <div className="p-5 border-b border-slate-800">
          <div className="flex items-center justify-between">
            <Link href="/vi/admin" className="flex items-center gap-2 group">
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            const isActive = pathname === item.href ||
              (item.href !== "/vi/admin" && pathname.startsWith(item.href));
            return (
