...
import { FolderPlus, Upload, RefreshCw, AlertCircle } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const handleDeleteFolder = async (folderId: string) => {
    try {
      // First, get folder details for confirmation
      ...
      const countSubfolders = (folders: FolderData[], parentId: string): number => {
        ...
        return children.length + children.reduce((sum, child) =>
          sum + countSubfolders(folders, child.id), 0
        );
      };

      const subfolderCount = countSubfolders(folders, folderId);

      // Get files count for this folder (we'll estimate since we don't have all files loaded)
      let fileCountMessage = "and all files within";
      ...
      if (response.ok) {
        ...
        await loadFolders();
        await loadFiles(); // Refresh files list as well
  ...
  const handleSearch = useCallback((query: string) => {
    ...
    setSelectedFolderId(null); // Clear folder selection when searching
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <Navbar user={user} onLogout={handleLogout} onOpenUserProfile={() => setShowUserProfile(true)} />
                ...
                <div className="mb-6">
                  ...
                  {createFolderParent && (
                    <p className="mt-1 text-sm text-gray-500">
                      Creating in: {folders.find(f => f.id === createFolderParent)?.name || "Unknown"}
                    </p>
                  )}
                </div>
