...
import { Eye, EyeOff, AlertTriangle, Zap, FileText, Image, Video, Music, Database, Archive, Box, Palette, Calendar, Mail, BarChart3, PenTool } from "lucide-react";
import { getPreviewCapability, canPreviewFile, isSecureForPreview, formatPreviewSizeLimit } from "@/lib/file-preview-utils";
...
export default function PreviewIndicator({
  ...
  const getTooltipContent = () => {
    ...
    if (!canPreview) {
      ...
      if (previewCapability.maxPreviewSize > 0 && fileSize > previewCapability.maxPreviewSize) {
...
export function PreviewStatusBadge({
  ...
  const colorClass = typeColors[previewCapability.previewType] || typeColors.unsupported;
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full ${colorClass}`}>
      ...
      {previewCapability.requiresSpecialHandling && (
