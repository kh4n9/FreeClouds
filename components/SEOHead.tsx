...
export default function SEOHead({
  ...
  const breadcrumbData = breadcrumbs.length > 0 ? generateBreadcrumbs(breadcrumbs) : null;
  ...
  const defaultHreflangLinks = hreflangLinks.length > 0 ? hreflangLinks : [
    {
      hrefLang: 'en',
...
export function DashboardPageSEO({ language = 'en' }: { language?: Language }) {
  return (
    <SEOHead
      ...
      breadcrumbs={[
        { name: language === 'vi' ? 'Trang Chủ' : 'Home', url: '/' },
        { name: language === 'vi' ? 'Bảng Điều Khiển' : 'Dashboard', url: '/dashboard' }
      ]}
    />
  );
}

export function ForgotPasswordPageSEO({ language = 'en' }: { language?: Language }) {
  return (
    <SEOHead
      title={language === 'vi'
      ...
      breadcrumbs={[
        { name: language === 'vi' ? 'Trang Chủ' : 'Home', url: '/' },
        { name: language === 'vi' ? 'Đặt Lại Mật Khẩu' : 'Reset Password', url: '/forgot-password' }
      ]}
    />
  );
}
